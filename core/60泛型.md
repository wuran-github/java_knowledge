# 泛型
<!-- TOC -->autoauto- [泛型](#泛型)auto    - [为什么要使用泛型](#为什么要使用泛型)auto        - [类型参数的好处](#类型参数的好处)auto        - [谁想成为泛型程序员](#谁想成为泛型程序员)auto    - [定义简单泛型类](#定义简单泛型类)auto    - [泛型方法](#泛型方法)auto    - [类型变量的限定](#类型变量的限定)auto    - [泛型代码和虚拟机](#泛型代码和虚拟机)auto        - [类型擦除](#类型擦除)auto        - [翻译泛型表达式](#翻译泛型表达式)auto        - [翻译泛型方法](#翻译泛型方法)auto            - [总结](#总结)auto        - [调用遗留代码](#调用遗留代码)auto    - [约束和局限性](#约束和局限性)auto        - [不能用基本类型实例化类型参数](#不能用基本类型实例化类型参数)auto        - [运行时类型查询只适用于原始类型](#运行时类型查询只适用于原始类型)auto        - [不能创建参数化类型的数组](#不能创建参数化类型的数组)auto        - [Varargs警告](#varargs警告)auto        - [不能实例化类型变量](#不能实例化类型变量)auto        - [不能构造泛型数组](#不能构造泛型数组)auto        - [泛型类的静态上下文中类型变量无效](#泛型类的静态上下文中类型变量无效)auto        - [不能抛出或捕获泛型类的实例](#不能抛出或捕获泛型类的实例)auto        - [可以消除对受查异常的检查](#可以消除对受查异常的检查)auto        - [注意擦除后的冲突](#注意擦除后的冲突)auto    - [泛型类型的继承规则](#泛型类型的继承规则)auto    - [通配符类型](#通配符类型)auto        - [通配符概念](#通配符概念)auto        - [通配符的超类型限定](#通配符的超类型限定)auto        - [无限定通配符](#无限定通配符)auto        - [通配符捕获](#通配符捕获)auto    - [反射和泛型](#反射和泛型)auto        - [泛型Class类](#泛型class类)autoauto<!-- /TOC -->
...
- 类型参数十分有用，这是因为它允许`Class<T>`方法的返回类型更加具有针对性。P354
